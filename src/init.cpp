 ...
 * between machinecoind, and machinecoin-qt and non-server tools.
*/
...
void Shutdown()
{
    ...
    RenameThread("machinecoin-shutoff");
...
static void OnRPCStopped()
{
    ...
    LogPrint(MCLog::RPC, "RPC stopped.\n");
}
...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-litemode=<n>", strprintf("Disable all Machinecoin specific functionality (Masternodes, Governance) (0-1, default: %u)", 0), true, OptionsCategory::MN);
    gArgs.AddArg("-masternode=<n>", strprintf("Enable the client to act as a masternode (0-1, default: %u)", 0), true, OptionsCategory::MN);
    gArgs.AddArg("-mnconf=<file>", strprintf("Specify masternode configuration file (default: %s)", "masternode.conf"), true, OptionsCategory::MN);
    gArgs.AddArg("-mnconflock=<n>", strprintf("Lock masternodes from masternode configuration file (default: %u)", 1), true, OptionsCategory::MN);
    gArgs.AddArg("-masternodeprivkey=<n>", "Set the masternode private key", true, OptionsCategory::MN);
...
static void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("machinecoin-loadblk");
...
/** Sanity checks
 *  Ensure that Machinecoin is running in a usable environment with all
...
{
    // Make sure only a single Machinecoin process is using the data directory.
...
bool AppInitMain()
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", MACHINECOIN_CONF_FILENAME)).string());
    LogPrintf("Using at most %i automatic connections (%i file descriptors available)\n", nMaxConnections, nFD);

    // Warn about relative -datadir path.
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if machinecoin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if machinecoin is started while in a temporary directory.\n",
